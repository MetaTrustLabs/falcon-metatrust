...
from falcon.core.solidity_types import TypeAlias
from falcon.core.solidity_types.array_type import ArrayType
from falcon.core.solidity_types.elementary_type import (
    ElementaryType,
    ElementaryTypeName,
)
...
from falcon.core.variables.function_type_variable import FunctionTypeVariable
from falcon.exceptions import FalconError
from falcon.solc_parsing.exceptions import ParsingError
from falcon.solc_parsing.expressions.expression_parsing import CallerContextExpression

if TYPE_CHECKING:
    from falcon.core.declarations import Structure, Enum
...
def _find_from_type_name(  # pylint: disable=too-many-locals,too-many-branches,too-many-statements,too-many-arguments
    ...
    if not var_type:
        ...
        # all_enums += contract.falcon.enums_top_level
    ...
    if not var_type:
        ...
        # all_structures += contract.falcon.structures_top_level
    ...
    if not var_type:
        for contract in contracts_direct_access:
            var_type = next((c for c in contract.compilation_unit.contracts if c.name == name),None)
            if var_type:
                break
        # raise ParsingError()
...
def _add_type_references(type_found: Type, src: str, sl: "FalconCompilationUnit"):
...
def parse_type(
    ...
    caller_context: Union[CallerContextExpression, "FalconCompilationUnitSolc"],
) -> Type:
    """
    caller_context can be a FalconCompilationUnitSolc because we recursively call the function
    and go up in the context's scope. If we are really lost we just go over the FalconCompilationUnitSolc
    ...
    from falcon.solc_parsing.expressions.expression_parsing import parse_expression
    from falcon.solc_parsing.variables.function_type_variable import FunctionTypeVariableSolc
    ...
    if isinstance(caller_context, FalconCompilationUnitSolc) or (
        isinstance(caller_context, FunctionSolc) and caller_context.contract_parser is None
    ):
        structures_direct_access: List["Structure"]
        if isinstance(caller_context, FalconCompilationUnitSolc):
            ...
            next_context = caller_context.falcon_parser
        ...
        all_structures += contract.structures
    ...
    if t[key] == "IdentifierPath":
        ...
        raise FalconError("Solidity 0.8 not supported with the legacy AST")
