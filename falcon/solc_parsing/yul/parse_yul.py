...
from falcon.core.cfg.node import NodeType, Node, link_nodes
...
from falcon.core.declarations.function import FunctionLanguage
...
from falcon.solc_parsing.yul.evm_functions import (
...
from falcon.solc_parsing.expressions.find_variable import find_top_level
from falcon.visitors.expression.find_calls import FindCalls
from falcon.visitors.expression.read_var import ReadVar
from falcon.visitors.expression.write_var import WriteVar
...
class YulFunction(YulScope):
    ...
    def new_node(self, node_type, src) -> YulNode:
        if self._function:
            ...
            raise FalconException("standalone yul objects are not supported yet")
...
def parse_yul_function_call(root: YulScope, node: YulNode, ast: Dict) -> Optional[Expression]:
    ...
    raise FalconException(f"unexpected function call target type {str(type(ident.value))}")
...
def _parse_yul_magic_suffixes(name: str, root: YulScope) -> Optional[Expression]:
    ...
    # Currently FalconIR doesnt support raw access to memory
    ...
    if name.endswith(".length"):
        ...
        # The code below is an hotfix to allow falcon to process length in yul
...
def parse_yul_identifier(root: YulScope, _node: YulNode, ast: Dict) -> Optional[Expression]:
    ...
    for f in root.contract.all_functions_called:
        if f.name == name:
            return Identifier(f)

    raise FalconException(f"unresolved reference to identifier {name}")
...
def parse_yul_unsupported(_root: YulScope, _node: YulNode, ast: Dict) -> Optional[Expression]:
    raise FalconException(f"no parser available for {ast['nodeType']} {json.dumps(ast, indent=2)}")
