...
from falcon.analyses.data_dependency.data_dependency import compute_dependency
...
from falcon.core.declarations.enum_top_level import EnumTopLevel
from falcon.core.declarations.function_top_level import FunctionTopLevel
from falcon.core.declarations.import_directive import Import
from falcon.core.declarations.pragma_directive import Pragma
from falcon.core.declarations.structure_top_level import StructureTopLevel
from falcon.core.scope.scope import FileScope
from falcon.core.solidity_types import ElementaryType, TypeAliasTopLevel
from falcon.core.variables.top_level_variable import TopLevelVariable
from falcon.exceptions import FalconException
from falcon.solc_parsing.declarations.contract import ContractSolc
from falcon.solc_parsing.declarations.custom_error import CustomErrorSolc
from falcon.solc_parsing.declarations.function import FunctionSolc
from falcon.solc_parsing.declarations.structure_top_level import StructureTopLevelSolc
from falcon.solc_parsing.exceptions import VariableNotFound
from falcon.solc_parsing.variables.top_level_variable import TopLevelVariableSolc

logging.basicConfig()
logger = logging.getLogger("FalconSolcParsing")
logger.setLevel(logging.INFO)

class InheritanceResolutionError(FalconException):
    pass
def _handle_import_aliases(
    ...
    for symbol_alias in symbol_aliases:
        if (
            "foreign" in symbol_alias and isinstance(symbol_alias["foreign"], dict)
...
class FalconCompilationUnitSolc:
    ...
    def parse_top_level_from_loaded_json(
        ...
        for top_level_data in data_loaded[self.get_children()]:
            if top_level_data[self.get_key()] == "ContractDefinition":
                ...
                get_imported_scope = import_directive.scope if import_directive.filename == "." else self.compilation_unit.get_scope(import_directive.filename)
                ...
                logging.warning(f"Top level {top_level_data[self.get_key()]} not supported")
                # raise FalconException(f"Top level {top_level_data[self.get_key()]} not supported")
    ...
    def parse_contracts(self):  # pylint: disable=too-many-statements,too-many-branches
        ...
        for contract in self._underlying_contract_to_parser:
            if (
                contract.name.startswith("FalconInternalTopLevelContract")
                and not contract.is_top_level
            ):
                raise FalconException(
                    # region multi-line-string
                    """Your codebase has a contract named 'FalconInternalTopLevelContract'.
Please rename it, this name is reserved for Falcon's internals"""
                ...
                if i in contract_parser.remapping:
                    ...
                    if target == contract_parser.underlying_contract:
                        raise InheritanceResolutionError(
                            "Could not resolve contract inheritance. This is likely caused by an import renaming that collides with existing names."
                    ...
                    if target:
                        ...
                        target = contract_parser.underlying_contract.file_scope.get_contract_from_id(i)
                        if target:
                            ancestors.append(target)
                        else:
    ...
    def analyze_contracts(self):  # pylint: disable=too-many-statements,too-many-branches
        if not self._parsed:
            raise FalconException("Parse the contract before running analyses")
    ...
    def _analyze_top_level_structures(self):
        try:
            ...
            raise FalconException(f"Missing struct {e} during top level structure analyze") from e
    ...
    def _analyze_top_level_variables(self):
        try:
            ...
            raise FalconException(f"Missing {e} during variable analyze") from e
    ...
    def _analyze_content_top_level_function(self):
        try:
            ...
            raise FalconException(f"Missing {e} during top level function analyze") from e
    ...
    def _convert_to_falconir(self):
        ...
        for contract in self._compilation_unit.contracts:
            ...
            contract.convert_expression_to_falconir_ssa()

        for func in self._compilation_unit.functions_top_level:
            func.generate_falconir_and_analyze()
            func.generate_falconir_ssa({})
