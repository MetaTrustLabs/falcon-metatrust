...
from falcon.core.cfg.node import NodeType, link_nodes, insert_node, Node
from falcon.core.cfg.scope import Scope
from falcon.core.declarations.contract import Contract
from falcon.core.declarations.function import (
    Function,
    ModifierStatements,
    FunctionType,
)
...
from falcon.core.variables.local_variable_init_from_tuple import LocalVariableInitFromTuple
from falcon.solc_parsing.cfg.node import NodeSolc
...
from falcon.solc_parsing.variables.local_variable_init_from_tuple import (
    LocalVariableInitFromTupleSolc,
)
from falcon.solc_parsing.variables.variable_declaration import MultipleVariablesDeclaration
from falcon.solc_parsing.yul.parse_yul import YulBlock
from falcon.utils.expression_manipulations import SplitTernaryExpression
from falcon.visitors.expression.export_values import ExportValues
from falcon.visitors.expression.has_conditional import HasConditional
...
class FunctionSolc(CallerContextExpression):
    ...
    def _parse_try_catch(self, statement: Dict, node: NodeSolc) -> NodeSolc:
        ...
        if externalCall is None:
            raise ParsingError(f"Try/Catch not correctly parsed by Falcon {statement}")
        catch_scope = Scope(
            node.underlying_node.scope.is_checked, False, node.underlying_node.scope
        )
    ...
    def _parse_catch(self, statement: Dict, node: NodeSolc) -> NodeSolc:
        ...
        if block is None:
            raise ParsingError(f"Catch not correctly parsed by Falcon {statement}")
    ...
    def _fix_catch(self, node: Node, end_node: Node, max_recursion=10):
        if not node.sons:
            link_nodes(node, end_node)
        else:
            for son in node.sons:
                if son != end_node and max_recursion > 0:
                    self._fix_catch(son, end_node, max_recursion=max_recursion - 1)
