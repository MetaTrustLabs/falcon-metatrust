...
from falcon.core.declarations.solidity_variables import SolidityCustomRevert
from falcon.core.expressions import Identifier, Literal
from falcon.core.solidity_types import (
    ArrayType,
    ElementaryType,
    FunctionType,
    MappingType,
...
from falcon.core.solidity_types.type_alias import TypeAlias
...
from falcon.ir.tmp_operations.argument import Argument, ArgumentType
from falcon.ir.tmp_operations.tmp_call import TmpCall
from falcon.ir.tmp_operations.tmp_new_array import TmpNewArray
from falcon.ir.tmp_operations.tmp_new_contract import TmpNewContract
from falcon.ir.tmp_operations.tmp_new_elementary_type import TmpNewElementaryType
from falcon.ir.tmp_operations.tmp_new_structure import TmpNewStructure
from falcon.ir.variables import Constant, ReferenceVariable, TemporaryVariable
from falcon.ir.variables import TupleVariable
from falcon.utils.function import get_function_id
from falcon.utils.type import export_nested_types_from_variable
from falcon.visitors.falconir.expression_to_falconir import ExpressionToFalconIR
...
def _convert_type_contract(ir, compilation_unit: "FalconCompilationUnit"):
    ...
    raise FalconIRError(f"type({contract.name}).{ir.variable_right} is unknown")
    ...
    if isinstance(ir, OperationWithLValue):
        ...
        if not ir.lvalue.type:
            if isinstance(ir, Assignment):
                ...
                #
                # return_addr().balance
                # Here falconIR will incorrectly create a REF variable instead of a Temp variable
                ...
                raise FalconIRError(f"Not handling {type(ir)} during type propagation")
    return None
...
def extract_tmp_call(ins: TmpCall, contract: Optional[Contract]):  # pylint: disable=too-many-locals
    ...
    if isinstance(ins.ori, Member):
        ...
        if isinstance(ins.ori.variable_left, Function):
            ...
            #     }
            # }
            to_log = "Falcon does not support dynamic functions to libraries if functions have the same name"
            to_log += f"{[candidate.full_name for candidate in candidates]}"
            raise FalconIRError(to_log)
...
def convert_to_low_level(ir):
    ...
    if ir.function_name in ["call", "delegatecall", "callcode", "staticcall"]:
        ...
        return new_ir
    raise FalconIRError(f"Incorrect conversion to low level {ir}")
...
def apply_ir_heuristics(irs, node):
    """
    Apply a set of heuristic to improve falconIR
    """
