...
from falcon.core.declarations import Contract, Enum, SolidityVariable, Function
...
class ReferenceVariable(ChildNode, Variable):
    def __init__(self, node: "Node", index=None):
        super().__init__()
        if index is None:
            self._index = node.compilation_unit.counter_falconir_reference
            node.compilation_unit.counter_falconir_reference += 1
        else:
            self._index = index
    ...
    def points_to(self, points_to):
        ...
        if is_valid_lvalue(points_to) or isinstance(points_to, (SolidityVariable, Contract, Enum)):
