...
###################################################################################
###################################################################################
# region FalconIR variables to SSA
###################################################################################
###################################################################################


def transform_falconir_vars_to_ssa(function):
    """
    Transform falconIR vars to SSA (TemporaryVariable, ReferenceVariable, TupleVariable)
    """
    variables = []
...
def last_name(n, var, init_vars):
    ...
    candidates = [x for x in candidates if hasattr(x, 'index')]
    if len(candidates) > 0:
    ...
    if isinstance(new_ir, OperationWithLValue):
        ...
        if isinstance(lvalue, (LocalIRVariable, StateIRVariable)):
            if isinstance(lvalue, LocalIRVariable):
                ...
                if not lvalue.canonical_name in all_state_variables_instances:
                    return
...
def copy_ir(ir, *instances):
    ...
    raise FalconIRError(f"Impossible ir copy on {ir} ({type(ir)})")
