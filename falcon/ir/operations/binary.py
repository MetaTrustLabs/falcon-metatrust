...
class BinaryType(Enum):
    ...
    def get_type(operation_type):  # pylint: disable=too-many-branches
        ...
        raise FalconIRError(f"get_type: Unknown operation type {operation_type})")

    def can_be_checked_for_undeflow(self):
        return self in [
            # BinaryType.POWER,
    ...
    def __str__(self):  # pylint: disable=too-many-branches
        ...
        if self == BinaryType.OROR:
            return "||"
        raise FalconIRError(f"str: Unknown operation type {self} {type(self)})")
