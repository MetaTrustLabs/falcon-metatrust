...
from falcon.core.context.context import Context
from falcon.core.declarations import Contract, FunctionContract
from falcon.core.declarations.top_level import TopLevel
from falcon.core.source_mapping.source_mapping import SourceMapping, Source
from falcon.ir.variables import Constant
from falcon.utils.colors import red
from falcon.utils.source_mapping import get_definition, get_references, get_implementation
...
class FalconCore(Context):
    """
    Falcon static analyzer
    """
    ...
    def __init__(self):
        ...
        self._previous_results_filename: str = "falcon.db.json"
        self._results_to_hide: List = []
        self._previous_results: List = []
        # From triaged result
        self._previous_results_ids: Set[str] = set()
        # Every falcon object has a list of result from detector
        ...
        # If set to true, falcon will not catch errors during parsing
        self._disallow_partial: bool = False
        self._skip_assembly: bool = False

        self._show_ignored_findings = False

        self._compilation_units: List[FalconCompilationUnit] = []
    ...
    @property
    def compilation_units(self) -> List[FalconCompilationUnit]:
        return list(self._compilation_units)

    def add_compilation_unit(self, compilation_unit: FalconCompilationUnit):
    ...
    def has_ignore_comment(self, r: Dict) -> bool:
        ...
        for file, lines in mapping_elements_with_lines:
            ...
            if ignore_line_text:
                match = re.findall(
                    r"^\s*//\s*falcon-disable-next-line\s*([a-zA-Z0-9_,-]*)",
        ...
        for path in self._paths_to_filter:
            try:
                ...
                logger.error(
                    ...
                    "\nFalcon supports the Python re format"
                )
