...
from falcon.ir.convert import convert_expression
...
from falcon.all_exceptions import FalconException
...
class Node(SourceMapping, ChildFunction):  # pylint: disable=too-many-public-methods
    ...
    def __init__(
        ...
        self._ssa_vars_written: List["FalconIRVariable"] = []
        self._ssa_vars_read: List["FalconIRVariable"] = []
        ...
        self._falconir_vars: Set["FalconIRVariable"] = set()  # non SSA
    ...
    @property
    def ssa_variables_read(self) -> List["FalconIRVariable"]:
        """
        list(Variable): Variables read (local/state/solidity)
        """
    ...
    def falconir_variables(self) -> List["FalconIRVariable"]:
        return list(self._falconir_vars)
    ...
    @property
    def ssa_variables_written(self) -> List["FalconIRVariable"]:
        """
        list(Variable): Variables written (local/state/solidity)
        """
    ...
    def irs(self) -> List[Operation]:
        """Returns the falconIR representation

        return
            list(falconIR.Operation)
        """
        return self._irs

    @property
    def irs_ssa(self) -> List[Operation]:
        """Returns the falconIR representation with SSA
        ...
        if self._all_falconir_operations is None:
            ...
            for ir in self.irs:
                if isinstance(ir, InternalCall):
                    irs += ir.function.all_falconir_operations()
            self._all_falconir_operations = irs
        return self._all_falconir_operations

    @staticmethod
    def _is_non_falconir_var(var: Variable):
        return not isinstance(var, (Constant, ReferenceVariable, TemporaryVariable, TupleVariable))

    @staticmethod
    def _is_valid_falconir_var(var: Variable):
    ...
    def _find_read_write_call(self):  # pylint: disable=too-many-statements
        ...
        for ir in self.irs:
            ...
            self._falconir_vars |= {v for v in ir.read if self._is_valid_falconir_var(v)}

            if isinstance(ir, OperationWithLValue):
                var = ir.lvalue
                if var and self._is_valid_falconir_var(var):
                    self._falconir_vars.add(var)

            if not isinstance(ir, (Phi, Index, Member)):
                self._vars_read += [v for v in ir.read if self._is_non_falconir_var(v)]
                for var in ir.read:
                ...
                if self._is_non_falconir_var(var):
                    self._vars_read.append(var)
                if isinstance(var, ReferenceVariable):
                    origin = var.points_to_origin
                    if self._is_non_falconir_var(origin):
            ...
            if isinstance(ir, OperationWithLValue):
                ...
                if var and self._is_non_falconir_var(var):
                ...
                if ir.destination.type:
                    if isinstance(ir.destination.type, Contract):
                        ...
                        try:
                            ...
                            try:
                                self._high_level_calls.append((ir.destination.type.underlying_type, ir.function))
                            except AttributeError as error:
    ...
    def update_read_write_using_ssa(self):
        ...
        for ir in self.irs_ssa:
            ...
            if isinstance(ir, OperationWithLValue):
                ...
                if isinstance(var, ReferenceVariable):
                    var = var.points_to_origin
                # Only store non-falconIR variables
