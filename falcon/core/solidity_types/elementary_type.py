...
class ElementaryType(Type):
    ...
    def size(self) -> int:
        ...
        raise FalconException(f"{t} does not have a size")
    ...
    def min(self) -> int:
        ...
        raise FalconException(f"{self.name} does not have a min value")

    @property
    def max(self) -> int:
        if self.name in MaxValues:
            return MaxValues[self.name]
        raise FalconException(f"{self.name} does not have a max value")
