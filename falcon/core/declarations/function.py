...
from falcon.core.cfg.scope import Scope
from falcon.core.declarations.solidity_variables import (
    SolidityFunction,
    SolidityVariable,
    SolidityVariableComposed,
)
from falcon.core.expressions import (
    Identifier,
    IndexAccess,
    MemberAccess,
    UnaryOperation,
...
from falcon.utils.type import convert_type_for_solidity_signature_to_string
from falcon.utils.utils import unroll

# pylint: disable=import-outside-toplevel,too-many-instance-attributes,too-many-statements,too-many-lines

if TYPE_CHECKING:
    from falcon.utils.type_helpers import (
        InternalCallType,
        LowLevelCallType,
        HighLevelCallType,
        LibraryCallType,
    ...
    from falcon.ir.operations import Operation
...
class Function(SourceMapping, metaclass=ABCMeta):  # pylint: disable=too-many-public-methods
    ...
    def __init__(self, compilation_unit: "FalconCompilationUnit"):
        ...
        self._falconir_variables: Set["FalconIRVariable"] = set()
        ...
        self._return_values: Optional[List["FalconIRVariable"]] = None
        self._return_values_ssa: Optional[List["FalconIRVariable"]] = None
        ...
        self._falconir_operations: Optional[List["Operation"]] = None
        self._falconir_ssa_operations: Optional[List["Operation"]] = None

        self._all_expressions: Optional[List["Expression"]] = None
        self._all_falconir_operations: Optional[List["Operation"]] = None
        ...
        self._all_falconir_variables: Optional[List["FalconIRVariable"]] = None
        ...
        self.compilation_unit: "FalconCompilationUnit" = compilation_unit
    ...
    def name(self) -> str:
        ...
        if self._function_type == FunctionType.CONSTRUCTOR_VARIABLES:
            return "falconConstructorVariables"
        if self._function_type == FunctionType.CONSTRUCTOR_CONSTANT_VARIABLES:
            return "falconConstructorConstantVariables"
        return self._name
    ...
    def can_reenter(self, callstack=None) -> bool:
        ...
        if self._can_reenter is None:
            ...
            for ir in self.all_falconir_operations():
    ...
    def is_constructor_variables(self) -> bool:
        ...
        Falcon has inbuilt functions to hold the state variables initialization
        """
    ...
    @property
    def falconir_variables(self) -> List["FalconIRVariable"]:
        """
        Temporary and Reference Variables (not SSA form)
        """

        return list(self._falconir_variables)
    ...
    @property
    def return_values(self) -> List["FalconIRVariable"]:
        """
        list(Return Values): List of the return values
        """
        from falcon.core.cfg.node import NodeType
        from falcon.ir.operations import Return
    ...
    @property
    def return_values_ssa(self) -> List["FalconIRVariable"]:
        """
    ...
    @property
    def falconir_operations(self) -> List["Operation"]:
        """
        list(Operation): List of the ir operations
        """
        if self._falconir_operations is None:
            operationss = [n.irs for n in self.nodes]
            operations = [item for sublist in operationss for item in sublist if item]
            self._falconir_operations = operations
        return self._falconir_operations

    @property
    def falconir_ssa_operations(self) -> List["Operation"]:
        """
        list(Operation): List of the ir operations (SSA)
        """
        if self._falconir_ssa_operations is None:
            operationss = [n.irs_ssa for n in self.nodes]
            operations = [item for sublist in operationss for item in sublist if item]
            self._falconir_ssa_operations = operations
        return self._falconir_ssa_operations
    ...
    def all_falconir_variables(self) -> List["FalconIRVariable"]:
        """recursive version of falconir_variables"""
        if self._all_falconir_variables is None:
            self._all_falconir_variables = self._explore_functions(lambda x: x.falconir_variables)
        return self._all_falconir_variables
    ...
    def all_falconir_operations(self) -> List["Operation"]:
        if self._all_falconir_operations is None:
            self._all_falconir_operations = self._explore_functions(lambda x: x.falconir_operations)
        return self._all_falconir_operations
    ...
    def falconir_cfg_to_dot(self, filename: str):
        """
        Export the CFG to a DOT file. The nodes includes the Solidity expressions and the IRs
        :param filename:
        :return:
        """
        content = self.falconir_cfg_to_dot_str()
        with open(filename, "w", encoding="utf8") as f:
            f.write(content)

    def falconir_cfg_to_dot_str(self, skip_expressions=False) -> str:
        """
    ...
    def _analyze_read_write(self):
        ...
        falconir_variables = [x.falconir_variables for x in self.nodes]
        falconir_variables = [x for x in falconir_variables if x]
        self._falconir_variables = [item for sublist in falconir_variables for item in sublist]
    ...
    ) -> Dict[str, Set["FalconIRVariable"]]:
        # pylint: disable=too-many-locals,too-many-branches
        from falcon.ir.variables import ReferenceVariable
        from falcon.ir.operations import OperationWithLValue
        from falcon.core.cfg.node import NodeType
    ...
    def _unchange_phi(ir: "Operation"):
        from falcon.ir.operations import Phi, PhiCallback
    ...
    def fix_phi(self, last_state_variables_instances, initial_state_variables_instances):
        from falcon.ir.operations import InternalCall, PhiCallback
        from falcon.ir.variables import Constant, StateIRVariable
    ...
    def generate_falconir_and_analyze(self):
        for node in self.nodes:
