...
from falcon.core.declarations.function import Function, FunctionType, FunctionLanguage
from falcon.utils.erc import (
    ERC20_signatures,
    ERC165_signatures,
    ERC223_signatures,
    ERC721_signatures,
...
from falcon.utils.tests_pattern import is_test_contract

# pylint: disable=too-many-lines,too-many-instance-attributes,import-outside-toplevel,too-many-nested-blocks
if TYPE_CHECKING:
    from falcon.utils.type_helpers import LibraryCallType, HighLevelCallType, InternalCallType
...
class Contract(SourceMapping):  # pylint: disable=too-many-public-methods
    ...
    def falconir_variables(self) -> List["FalconIRVariable"]:
        ...
        """
        falconir_variabless = [f.falconir_variables for f in self.functions + self.modifiers]  # type: ignore
        falconir_variables = [item for sublist in falconir_variabless for item in sublist]
        return list(set(falconir_variables))
    ...
    def is_incorrectly_constructed(self) -> bool:
        """
        Return true if there was an internal Falcon's issue when analyzing the contract
        :return:
        """
    ...
    def convert_expression_to_falconir_ssa(self):
        """
        Assume generate_falconir_and_analyze was called on all functions

        :return:
        """
        from falcon.ir.variables import StateIRVariable
        ...
        for func in self.functions + self.modifiers:
            func.generate_falconir_ssa(all_ssa_state_variables_instances)
        ...
        for func in self.functions + self.modifiers:
            ...
            for variable_name, instances in result.items():
                if variable_name in last_state_variables_instances:
                    if isinstance(last_state_variables_instances[variable_name], set):
                        last_state_variables_instances[variable_name].update(instances)
                    else:
