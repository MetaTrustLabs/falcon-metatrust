...
from falcon.core.declarations import Contract, Import, Pragma
...
class FileScope:
    ...
    def get_contract_from_name(self, name: Union[str, Constant]) -> Optional[Contract]:
        contract = self.contracts.get(name.name if isinstance(name, Constant) else name, None)
        if not contract and len(self.contracts.values()) > 0:
            if isinstance(name, Constant):
                name = name.name
            for c in list(self.contracts.values())[0].compilation_unit.contracts:
                if c.name == name:
                    return c
        return contract

    def get_contract_from_id(self, id: int) -> Optional[Contract]:
        contract_id_map = dict(zip([x.id for x in self.contracts.values()], [x for x in self.contracts.values()]))
        return contract_id_map.get(id, None)
