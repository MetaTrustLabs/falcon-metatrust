...
from falcon.ir.operations import InternalDynamicCall, OperationWithLValue
...
def _is_vulnerable(node, variables_entrance):
    ...
    for ir_ssa in node.irs_ssa:
        if isinstance(ir_ssa, InternalDynamicCall):
            ...
            if isinstance(destination, FalconIRVariable) and ir_ssa.function.index == 0:
                return True
...
class UninitializedFunctionPtrsConstructor(AbstractDetector):
    ...
    ARGUMENT = "uninitialized-func-pointer"
