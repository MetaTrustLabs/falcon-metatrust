...
class TooManyDigits(AbstractDetector):
    ...
    def is_hex_address(value) -> bool:
        ...
        """
        return TooManyDigits._HEX_ADDRESS_REGEXP.fullmatch(value) is not None
            ...
            for ir in node.irs:
                ...
                for read in ir.read:
                    ...
                    if isinstance(read, Constant):
                        ...
                        if ("000" in value_as_str) or (TooManyDigits.is_hex_address(value_as_str)) or ("115792089" in value_as_str) or len(value_as_str)>3:
                            continue
                        else:
                            try:
