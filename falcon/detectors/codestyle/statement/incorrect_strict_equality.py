...
class IncorrectStrictEquality(AbstractDetector):
    ...
    HELP = "Dangerous strict equalities without msg.sender check in modifier"
    IMPACT = DetectorClassification.MEDIUM
    CONFIDENCE = DetectorClassification.HIGH

    WIKI = " "

    WIKI_TITLE = "Dangerous strict equalities without msg.sender check in modifier"
...
`Crowdsale` reaches 100 Ether. Bob sends 0.1 Ether. As a result, `fund_reached` is always false and the `crowdsale` never ends."""
    ...
    WIKI_RECOMMENDATION = (
        """Don't use strict equality to determine if an account has enough Ether or tokens. Or must have modifier to access controll"""
    )
    ...
    def tainted_equality_nodes(self, funcs, taints):
        ...
        for func in funcs:
            if len(func.modifiers) > 0 and ModifierUtil.check_all_modifiers_if_access_controll(func):
                continue
