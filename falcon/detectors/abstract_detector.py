...
from falcon.utils.colors import green, yellow, red
from falcon.formatters.exceptions import FormatImpossible
from falcon.formatters.utils.patches import apply_patch, create_diff
from falcon.utils.comparable_enum import ComparableEnum
from falcon.utils.output import Output, SupportedOutput
...
classification_colors: Dict[DetectorClassification, Callable[[str], str]] = {
    ...
    DetectorClassification.CRITICAL: red,
}
...
classification_txt = {
    ...
    DetectorClassification.CRITICAL: "Critical"
}


class AbstractDetector(metaclass=abc.ABCMeta):
    ARGUMENT = ""  # run the detector with falcon.py --ARGUMENT
    ...
    def __init__(
            self, compilation_unit: FalconCompilationUnit, falcon: "Falcon", logger: Logger
    ):
        ...
        if self.IMPACT not in [
            ...
            DetectorClassification.INFORMATIONAL,
            DetectorClassification.OPTIMIZATION,
            DetectorClassification.CRITICAL
        ]:
            raise IncorrectDetectorInitialization(
    ...
    def detect(self) -> List[Dict]:
        ...
        for r in [output.data for output in self._detect()]:
            key = f'{r.get("check")}-{r.get("first_markdown_element")}'
            if self.compilation_unit.core.valid_result(r) and r not in results and key not in result_keys:
                results.append(r)
        ...
        if results and self.falcon.triage_mode:
            while True:
                ...
                try:
                    ...
                    self.falcon.save_results_to_hide(
    ...
    def generate_result(
        ...
        output = Output(
            ...
            markdown_root=self.falcon.markdown_root,
        )
    ...
    @staticmethod
    def _format(_compilation_unit: FalconCompilationUnit, _result: Dict) -> None:
        """Implement format"""
        return
