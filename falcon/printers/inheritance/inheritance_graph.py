...
from falcon.utils.inheritance_analysis import (
...
class PrinterInheritanceGraph(AbstractPrinter):
    ...
    def __init__(self, falcon, logger):
        super().__init__(falcon, logger)

        inheritance = [x.inheritance for x in falcon.contracts]
        self.inheritance = {item for sublist in inheritance for item in sublist}

        self.overshadowing_state_variables = {}
        shadows = detect_state_variable_shadowing(falcon.contracts)
