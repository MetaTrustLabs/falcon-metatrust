...
from falcon.core.declarations import SolidityFunction, Function
...
from falcon.utils.code_complexity import compute_cyclomatic_complexity
from falcon.utils.colors import green, red, yellow
from falcon.utils.myprettytable import MyPrettyTable
from falcon.utils.standard_libraries import is_standard_library
from falcon.core.cfg.node import NodeType
from falcon.utils.tests_pattern import is_test_file
...
class PrinterHumanSummary(AbstractPrinter):
    ...
    def _get_detectors_result(self) -> Tuple[List[Dict], int, int, int, int, int]:
        ...
        checks_optimization = self.falcon.detectors_optimization
        checks_informational = self.falcon.detectors_informational
        checks_low = self.falcon.detectors_low
        checks_medium = self.falcon.detectors_medium
        checks_high = self.falcon.detectors_high
    ...
    def _lines_number(self):
        if not self.falcon.source_code:
            return None
        total_dep_lines = 0
        total_lines = 0
        total_tests_lines = 0

        for filename, source_code in self.falcon.source_code.items():
            lines = len(source_code.splitlines())
            is_dep = False
            if self.falcon.crytic_compile:
                is_dep = self.falcon.crytic_compile.is_dependency(filename)
            if is_dep:
    ...
    def _compilation_type(self):
        if self.falcon.crytic_compile is None:
            return "Compilation non standard\n"
        return f"Compiled with {str(self.falcon.crytic_compile.type)}\n"

    def _number_contracts(self):
        if self.falcon.crytic_compile is None:
            return len(self.falcon.contracts), 0, 0
        contracts = [c for c in self.falcon.contracts if not c.is_top_level]
