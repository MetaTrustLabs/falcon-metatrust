...
from falcon.detectors import all_detectors
from falcon.detectors.abstract_detector import AbstractDetector, DetectorClassification
from falcon.printers import all_printers
from falcon.printers.abstract_printer import AbstractPrinter
from falcon.falcon import Falcon
from falcon.utils.output import output_to_json, output_to_zip, output_to_sarif, ZIP_TYPES_ACCEPTED
from falcon.utils.output_capture import StandardOutputCapture
from falcon.utils.colors import red, set_colorization_enabled
from falcon.utils.command_line import (
    output_detectors,
...
def process_single(
...
) -> Tuple[Falcon, List[Dict], List[Dict], int]:
    """
    The core high-level code for running Falcon static analysis.
    ...
    print("begin to process ast")
    falcon = Falcon(target, ast_format=ast, **vars(args))
    print("run process")
    return _process(falcon, detector_classes, printer_classes)
...
def process_all(
    ...
    detector_classes: List[AbstractDetector],
    printer_classes: List[AbstractPrinter],
) -> Tuple[List[Falcon], List[Dict], List[Dict], int]:
    print("before compile_all")
    compilations = compile_all(target, **vars(args))
    print("after compile all")
    falcon_instances = []
    results_detectors = []
    results_printers = []
    analyzed_contracts_count = 0
    print("before: for compilation in compilations:")
    for compilation in compilations:
        ...
        falcon_instances.append(falcon)
        analyzed_contracts_count += current_analyzed_count
    print("after for recrusion")
    return (
        falcon_instances,
        results_detectors,
        results_printers,
...
def _process(
    ...
    for detector_cls in detector_classes:
        falcon.register_detector(detector_cls)
    print("check printer_cls")
    for printer_cls in printer_classes:
        falcon.register_printer(printer_cls)

    analyzed_contracts_count = len(falcon.contracts)

    results_detectors = []
    results_printers = []

    if not printer_classes:
        detector_results = falcon.run_detectors()
        detector_results = [x for x in detector_results if x]  # remove empty results
        detector_results = [item for sublist in detector_results for item in sublist]  # flatten
        results_detectors.extend(detector_results)

    else:
        printer_results = falcon.run_printers()
        printer_results = [x for x in printer_results if x]  # remove empty results
        results_printers.extend(printer_results)

    return falcon, results_detectors, results_printers, analyzed_contracts_count
...
def get_detectors_and_printers():
    ...
    for entry_point in iter_entry_points(group="falcon_analyzer.plugin", name=None):
        make_plugin = entry_point.load()
        make_plugin_functions.append(make_plugin)

    # try:
    #     from plugins.falcon import make_plugin as mt_make_plugin
    #     make_plugin_functions.append(mt_make_plugin)
    # except Exception as e:
    #     pass

    # handle plugins make_plugin call 
    for make_plugin in make_plugin_functions:
...
def parse_args(detector_classes, printer_classes):  # pylint: disable=too-many-statements
    ...
    parser.add_argument(
        ...
        version=require("falcon-analyzer")[0].version,
        action="version",
    )
    parser.add_argument(
        "--gptversion", 
        help="Choose to use gpt3 or gpt4",
        action="store",
        dest="gptversion",
    )
    parser.add_argument(
        "--skip", 
        help="skip for specific rule",
        action="store",
        dest="skip",
    )
    ...
    group_misc.add_argument(
        "--triage-mode",
        help="Run triage mode (save results in falcon.db.json)",
        action="store_true",
        dest="triage_mode",
        default=False,
    )
    ...
    group_misc.add_argument(
        "--config-file",
        help="Provide a config file (default: falcon.config.json)",
        action="store",
        dest="config_file",
        default=None,
    )
    ...
    parser.add_argument(
        ...
        help=argparse.SUPPRESS,
        action="store",
        default=defaults_flag_in_config["log_level"]
    )
...
def main():
    ...
    print("before get_detectors_and_printers")
    detectors, printers = get_detectors_and_printers()
    print("before main_impl")
...
def main_impl(
    ...
    """
    # Set logger of Falcon to info, to catch warnings related to the arg parsing
    ...
    if args.perf:
        cp = cProfile.Profile()
        cp.enable()
    print("set_colorization_enabled")
    ...
    print("before choose_printers")
    printer_classes = choose_printers(args, all_printer_classes)
    detector_classes = choose_detectors(args, all_detector_classes)

    print("before getLevelName")
    default_log = logging.getLevelName(args.log_level) if args.log_level else logging.INFO
    default_log = default_log if not args.debug else logging.DEBUG
    ...
    print("before StreamHandler")
    console_handler = logging.StreamHandler()
    console_handler.setLevel(logging.INFO)

    print("before FormatterCryticCompile")
    console_handler.setFormatter(FormatterCryticCompile())

    print("before getLogger")
    ...
    try:
        ...
        if args.solc_ast or (filename.endswith(".json") and not is_supported(filename)):
            ...
            if args.splitted:
                ...
                falcon_instances.append(falcon_instance)
            else:
                print("try3")
                for filename in filenames:
                    (
        ...
        if outputting_json or outputting_zip or output_to_sarif:
            ...
            if "compilations" in args.json_types:
                compilation_results = []
                for falcon_instance in falcon_instances:
                    compilation_results.append(
                        generate_standard_export(falcon_instance.crytic_compile)
                    )
    ...
    except FalconException as falcon_exception:
        output_error = str(falcon_exception)
        traceback.print_exc()
