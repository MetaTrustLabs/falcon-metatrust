...
from falcon.analyses.evm.evm_cfg_builder import load_evm_cfg_builder
...
def get_evm_instructions(obj):
    ...
    if KEY_EVM_INS not in obj.context:
        ...
        if not falcon.crytic_compile:
            raise FalconError("EVM features require to compile with crytic-compile")
        ...
        contract_info["bytecode_runtime"] = falcon.crytic_compile.bytecode_runtime(
            contract_info["contract"].name
        )
        contract_info["srcmap_runtime"] = falcon.crytic_compile.srcmap_runtime(
            contract_info["contract"].name
        )
        contract_info["cfg"] = CFG(contract_info["bytecode_runtime"])

        # Get contract init bytecode, srcmap and cfg
        contract_info["bytecode_init"] = falcon.crytic_compile.bytecode_init(
            contract_info["contract"].name
        )
        contract_info["srcmap_init"] = falcon.crytic_compile.srcmap_init(
            contract_info["contract"].name
        )
...
def _get_evm_instructions_function(function_info):
    ...
    if function_evm is None:
        ...
        logger.error(to_log)
        raise FalconError("Function " + function.name + " not found in the EVM code")
...
def generate_source_to_evm_ins_mapping(evm_instructions, srcmap_runtime, falcon, filename):
    """
    ...
    file_source = falcon.source_code[filename].encode("utf-8")
    prev_mapping = []
