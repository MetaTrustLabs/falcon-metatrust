...
from falcon.core.solidity_types import UserDefinedType, MappingType
from falcon.core.declarations import Enum, Contract, Structure, Function
from falcon.core.solidity_types.elementary_type import ElementaryTypeName
from falcon.core.variables.local_variable import LocalVariable
from falcon.formatters.exceptions import FormatError, FormatImpossible
from falcon.formatters.utils.patches import create_patch

logging.basicConfig(level=logging.INFO)
logger = logging.getLogger("Falcon.Format")


# pylint: disable=anomalous-backslash-in-string


def custom_format(compilation_unit: FalconCompilationUnit, result):
...
def _name_already_use(falcon, name):
    # Do not convert to a name used somewhere else
    if not KEY in falcon.context:
        all_names = set()
        for contract in falcon.contracts_derived:
        ...
        falcon.context[KEY] = all_names
    return name in falcon.context[KEY]


def _convert_CapWords(original_name, falcon):
    ...
    if _name_already_use(falcon, name):
...
def _convert_mixedCase(original_name, compilation_unit: FalconCompilationUnit):
    name = original_name
...
def _convert_UPPER_CASE_WITH_UNDERSCORES(name, compilation_unit: FalconCompilationUnit):
...
def _get_from_contract(compilation_unit: FalconCompilationUnit, element, name, getter):
...
def _patch(compilation_unit: FalconCompilationUnit, result, element, _target):
    scope = compilation_unit.get_scope(element["source_mapping"]["filename_absolute"])
...
def _is_var_declaration(falcon, filename, start):
    """
    ...
    """
    v = "var "
    return falcon.source_code[filename][start: start + len(v)] == v


def _explore_type(  # pylint: disable=too-many-arguments,too-many-locals,too-many-branches
    falcon, result, target, convert, custom_type, filename_source_code, start, end
):
    if isinstance(custom_type, UserDefinedType):
        ...
        if isinstance(custom_type.type, (Enum, Contract)):
            if custom_type.type == target:
                ...
                new_str = convert(old_str, falcon)

                loc_start = start
                if _is_var_declaration(falcon, filename_source_code, start):
            ...
            _explore_variables_declaration(
                falcon, custom_type.type.elems.values(), result, target, convert
            )
    ...
    if isinstance(custom_type, MappingType):
        ...
        if isinstance(custom_type.type_to, (UserDefinedType, MappingType)) or target in [
            ...
            full_txt_end = end
            full_txt = falcon.source_code[filename_source_code].encode("utf8")[
...
def _explore_variables_declaration(  # pylint: disable=too-many-arguments,too-many-locals,too-many-nested-blocks
    falcon, variables, result, target, convert, patch_comment=False
):
            ...
            if patch_comment and isinstance(variable, LocalVariable):
                if variable.source_mapping.lines:
                    ...
                    if variable in func.parameters:
                        ...
                        potential_comments = falcon.source_code[filename_source_code].encode(
                            "utf8"
                        )
...
def _explore_structures_declaration(falcon, structures, result, target, convert):
    for st in structures:
        # Explore the variable declared within the structure (VariableStructure)
        _explore_variables_declaration(falcon, st.elems.values(), result, target, convert)
...
def _explore_events_declaration(falcon, events, result, target, convert):
    for event in events:
        # Explore the parameters
        _explore_variables_declaration(falcon, event.elems, result, target, convert)
...
def _explore_irs(falcon, irs: List[Operation], result, target, convert):
...
def _explore_functions(falcon, functions, result, target, convert):
    for function in functions:
        _explore_variables_declaration(falcon, function.variables, result, target, convert, True)
        _explore_irs(falcon, function.all_falconir_operations(), result, target, convert)
...
def _explore_enums(falcon, enums, result, target, convert):
    for enum in enums:
        if enum == target:
            old_str = enum.name
            new_str = convert(old_str, falcon)
...
def _explore_contract(falcon, contract, result, target, convert):
    _explore_variables_declaration(falcon, contract.state_variables, result, target, convert)
    _explore_structures_declaration(falcon, contract.structures, result, target, convert)
    _explore_functions(falcon, contract.functions_and_modifiers, result, target, convert)
    _explore_enums(falcon, contract.enums, result, target, convert)
...
def _explore(compilation_unit: FalconCompilationUnit, result, target, convert):
