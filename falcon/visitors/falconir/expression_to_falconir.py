...
from falcon.core.solidity_types import ArrayType, ElementaryType, TypeAlias
...
from falcon.ir.tmp_operations.argument import Argument
...
logger = logging.getLogger("VISTIOR:ExpressionToFalconIR")

key = "expressionToFalconIR"
...
def convert_assignment(left, right, t, return_type):
    ...
    raise FalconIRError("Missing type during assignment conversion")


class ExpressionToFalconIR(ExpressionVisitor):
    def __init__(self, expression, node):  # pylint: disable=super-init-not-called
        from falcon.core.cfg.node import NodeType  # pylint: disable=import-outside-toplevel
    ...
    def _post_unary_operation(
        ...
        if expression.type in [UnaryOperationType.BANG, UnaryOperationType.TILD]:
            ...
            raise FalconIRError(f"Unary operation to IR not supported {expression}")
