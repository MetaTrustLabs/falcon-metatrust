...
def resolve_function(falcon, contract_name, function_name):
    """
    ...
    contracts = falcon.get_contract_from_name(contract_name)
...
def resolve_functions(falcon, functions):
    """
    ...
    for item in functions:
        if isinstance(item, str):
            ...
            resolved.append(resolve_function(falcon, parts[0], parts[1]))
            ...
            resolved.append(resolve_function(falcon, item[0], item[1]))
        else:
            raise ResolveFunctionException(
...
def __find_target_paths(falcon, target_function, current_path=None):
    ...
    for contract in falcon.contracts:
        for function in contract.functions_and_modifiers_declared:
            ...
            if all_target_functions.intersection(called_functions):
                path_results = __find_target_paths(falcon, function, current_path.copy())
                if path_results:
...
def find_target_paths(falcon, target_functions):
    """
    ...
    for target_function in target_functions:
        results = results.union(__find_target_paths(falcon, target_function))
