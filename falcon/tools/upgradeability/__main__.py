...
from falcon.utils.output import output_to_json
from falcon.tools.upgradeability.checks import all_checks
from falcon.tools.upgradeability.checks.abstract_checks import AbstractCheck
from falcon.tools.upgradeability.utils.command_line import (
...
logging.basicConfig()
logger: logging.Logger = logging.getLogger("Falcon")
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=" ",
        usage="falcon-check-upgradeability contract.sol ContractName",
    )
...
def main():
    ...
    try:
        variable1 = Falcon(v1_filename, **vars(args))
        ...
        if args.proxy_name:
            if args.proxy_filename:
                proxy = Falcon(args.proxy_filename, **vars(args))
            else:
                proxy = variable1
        ...
        if args.new_contract_name:
            if args.new_contract_filename:
                variable2 = Falcon(args.new_contract_filename, **vars(args))
            else:
                variable2 = variable1
        ...
        logger.error(str(falcon_exception))
        if args.json:
            output_to_json(args.json, str(falcon_exception), json_results)
        return
