...
class AbstractMutator(metaclass=abc.ABCMeta):  # pylint: disable=too-few-public-methods
    ...
    def __init__(self, falcon: Falcon, rate: int = 10, seed: Optional[int] = None):
        self.falcon = falcon
        self.seed = seed
        self.rate = rate
    ...
    def mutate(self) -> None:
        ...
        for file in all_patches["patches"]:
            original_txt = self.falcon.source_code[file].encode("utf8")
            ...
            diff = create_diff(self.falcon, original_txt, patched_txt, file)
            if not diff:
