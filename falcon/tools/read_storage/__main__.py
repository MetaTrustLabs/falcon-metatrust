...
from falcon.tools.read_storage.read_storage import FalconReadStorage
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        ...
        usage=(
            ...
            + "\tfalcon-read-storage $TARGET address --variable-name $NAME\n"
            + "To retrieve a contract's storage layout:\n"
            + "\tfalcon-read-storage $TARGET address --contract-name $NAME --layout\n"
            + "To retrieve a contract's storage layout and values:\n"
            + "\tfalcon-read-storage $TARGET address --contract-name $NAME --layout --value\n"
...
def main() -> None:
    ...
    if len(args.contract_source) == 2:
        ...
        falcon = Falcon(source_code, **vars(args))
    else:
        # Source code is published and retrieved via etherscan
        target = args.contract_source[0]
        falcon = Falcon(target, **vars(args))

    if args.contract_name:
        contracts = falcon.get_contract_from_name(args.contract_name)
    else:
        contracts = falcon.contracts

    srs = FalconReadStorage(contracts, args.max_depth)
