...
from falcon.tools.similarity.encode import load_and_encode, parse_target
from falcon.tools.similarity.model import load_model
...
logger = logging.getLogger("Falcon-simil")
...
def plot(args):  # pylint: disable=too-many-locals
    ...
    if decomposition is None or plt is None:
        logger.error(
            "ERROR: In order to use plot mode in falcon-simil, you need to install sklearn and matplotlib:"
        )
