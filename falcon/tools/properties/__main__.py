...
from falcon.tools.properties.properties.erc20 import generate_erc20, ERC20_PROPERTIES
from falcon.tools.properties.addresses.address import (
    Addresses,
    OWNER_ADDRESS,
    USER_ADDRESS,
    ATTACKER_ADDRESS,
...
def main():
    ...
    falcon = Falcon(args.filename, **vars(args))

    contracts = falcon.get_contract_from_name(args.contract)
    if len(contracts) != 1:
        if len(falcon.contracts) == 1:
            contract = falcon.contracts[0]
        else:
