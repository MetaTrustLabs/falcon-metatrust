...
from falcon.tools.properties.addresses.address import Addresses
from falcon.tools.properties.platforms.echidna import generate_echidna_config
from falcon.tools.properties.properties.ercs.erc20.properties.burn import (
    ERC20_NotBurnable,
    ERC20_Burnable,
)
from falcon.tools.properties.properties.ercs.erc20.properties.initialization import ERC20_CONFIG
from falcon.tools.properties.properties.ercs.erc20.properties.mint import ERC20_NotMintable
from falcon.tools.properties.properties.ercs.erc20.properties.mint_and_burn import (
    ERC20_NotMintableNotBurnable,
)
from falcon.tools.properties.properties.ercs.erc20.properties.transfer import (
    ERC20_Transferable,
    ERC20_Pausable,
)
from falcon.tools.properties.properties.ercs.erc20.unit_tests.truffle import generate_truffle_test
from falcon.tools.properties.properties.properties import (
    property_to_solidity,
    Property,
)
from falcon.tools.properties.solidity.generate_properties import (
    generate_solidity_properties,
    generate_test_contract,
    generate_solidity_interface,
)
from falcon.utils.colors import red, green
...
def generate_erc20(
    ...
    if contract.compilation_unit.core.crytic_compile.type not in [
        ...
        logging.error(
            f"{contract.compilation_unit.core.crytic_compile.type} not yet supported by falcon-prop"
        )
        return
...
def _check_compatibility(contract):
    ...
    if not contract.is_erc20():
        errors = f"{contract} is not ERC20 compliant. Consider checking the contract with falcon-check-erc"
        return errors

    transfer = contract.get_function_from_signature("transfer(address,uint256)")

    if transfer.visibility != "public":
        errors = f"falcon-prop requires {transfer.canonical_name} to be public. Please change the visibility"
    ...
    if transfer_from.visibility != "public":
        if errors:
            errors += "\n"
        errors += f"falcon-prop requires {transfer_from.canonical_name} to be public. Please change the visibility"

    approve = contract.get_function_from_signature("approve(address,uint256)")
    if approve.visibility != "public":
        if errors:
            errors += "\n"
        errors += f"falcon-prop requires {approve.canonical_name} to be public. Please change the visibility"
