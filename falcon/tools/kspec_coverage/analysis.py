...
from falcon.utils.colors import yellow, green, red
from falcon.utils import output

logging.basicConfig(level=logging.WARNING)
logger = logging.getLogger("Falcon.kspec")
...
def _get_falcon_functions(falcon):
    ...
    all_functions_declared += list(
        {s for s in falcon.state_variables if s.visibility in ["public", "external"]}
    )
    falcon_functions = {
...
def _run_coverage_analysis(args, falcon, kspec_functions):
    # Collect all falcon functions
    falcon_functions = _get_falcon_functions(falcon)

    # Determine which klab specs were not resolved.
    falcon_functions_set = set(falcon_functions)
    kspec_functions_resolved = kspec_functions & falcon_functions_set
    kspec_functions_unresolved = kspec_functions - kspec_functions_resolved

    kspec_missing = []
    kspec_present = []

    for falcon_func_desc in sorted(falcon_functions_set):
        falcon_func = falcon_functions[falcon_func_desc]

        if falcon_func_desc in kspec_functions:
            kspec_present.append(falcon_func)
        else:
            kspec_missing.append(falcon_func)
...
def run_analysis(args, falcon, kspec_arg):
    ...
    _run_coverage_analysis(args, falcon, kspec_functions)
